...
ngx_http_iad_exec_iad(ngx_http_request_t *r,
    ...
    for (i = 0; i < computed_args->nelts; i++) {
        ...
        size_t str_zz_len =  data_map_value->len + 60;
        u_char *str_zz_data_p = ngx_palloc(r->pool,str_zz_len);
        
        
 
        if(data_map_value->len > 0){
            (void) ngx_snprintf(str_zz_data_p, str_zz_len,
...
static ngx_str_t* ngx_http_iad_timestamp(ngx_http_request_t *r)
{
    ...
    ngx_timestamp = ngx_palloc(r->pool,sizeof(ngx_str_t));
