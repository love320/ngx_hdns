...
int ngx_shmap_get_int32(ngx_shm_zone_t* zone, ngx_str_t* key, int32_t* i)
{
	...
	if(ret == 0){
		if(value_type != VT_INT32){
			ret = -1;
			//NLOG_ERROR("ngx_shmap_get_int32(key=%V) return invalid value_type=%d",key, value_type);
		}else{
...
ngx_shmap_incr(lua_State *L)
{
   ...
   // dd("looking up key %.*s in shared dict %.*s", (int) key->len, key->data,(int) ctx->name.len, ctx->name.data);
